...
**Design.** The interface follows the [Lamborghini DESIGN.md](https://github.com/VoltAgent/awesome-design-md/tree/main/design-md/lamborghini) from awesome-design-md: a true-black canvas, charcoal panels, white uppercase condensed headings (Barlow / Barlow Condensed stand in for the proprietary typeface), zero corner radius, no shadows, and gold reserved for the primary buttons. It is dark only. Chart colors follow a data-viz palette re-validated on the black surface for color-blind separation and contrast: the target stock is gold, reference stocks take a fixed order of distinct hues, context lines are gray, and candles are monochrome (hollow = up, filled = down). Every chart has a **Table** tab with the same numbers.
